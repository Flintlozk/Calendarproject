...
header("Pragma:no-cache");
	...
	if($_POST["singleDate"] == "" && $_POST["betweenDate_1"] == "" && $_POST["betweemDate_2"] == "" && $_POST['textuser_id'] == "" ){
			$sql=mysql_query("select username,DATE_FORMAT(starttime,'%d-%m-%Y'),time_in,location,employee,carname,subject,DATE_FORMAT(endtime,'%d-%m-%Y %H:%i:%s'),namecus,make  
...
<? if($_POST["singleDate"] !=""){?> 
			...
			<? }else if($_POST["textuser_id"] !=""){ ?>
            	<strong>รายงานวันที่ <?php echo DateThai($strDate);?> ทั้งหมด <?php echo number_format($num);?> รายการ </strong><br />
			<? }?>
